...
def process(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_fps, mp4_crf, resolution, lora_file, lora_multiplier):
    global stream
    assert input_image is not None, 'No input image!'
    ...
    async_run(worker, input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache,  mp4_fps, mp4_crf, resolution, lora_file, lora_multiplier)
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Group():
                ...
                mp4_fps = gr.Slider(label="FPS", minimum=1, maximum=60, value=30, step=1, info="Playback FPS for the output MP4.")
    ...
    ips = [input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_fps, mp4_crf, resolution, lora_file, lora_multiplier]
